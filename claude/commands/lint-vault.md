...
2. Read the vault's `INDEX.md` Rules section to get its knowledge-note folders, then run:
   `~/git/oleksiimorozenko/dotfiles/claude/bin/vault-lint.py ~/obsidian/<ctx> --knowledge-dirs <from INDEX rules> $ARGUMENTS`
   (assumes the dotfiles clone lives at `~/git/<user>/dotfiles`; adjust the path if it does not)
